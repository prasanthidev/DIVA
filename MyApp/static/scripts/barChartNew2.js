...
function addHtmlElements()
{
    ...
    height = divElement.clientHeight - 50
    //console.log(width)
    //console.log(height)
    ...
    gPyramidPlot.append('rect')
        ...
        .attr('height',gPyramidPlotHeight + 50)
        .attr('fill','white') 
        .attr('stroke','black')
        .attr('stroke-width','1')  
    
    gCountryName = gPyramidPlot.append("g")
                                .attr('transform','translate('+(0.5*gPyramidPlotWidth)+','+30+')')
    store.gCountryName = gCountryName
    gCountryName.append('text')
                .attr('id','CountryName')
                .text("United States")

    gLegend = gPyramidPlot.append('g')
    ...
    textLessGreater.attr('id','lessGreater')
                    ...
                    .attr('font-size','4em')
                    .attr('transform','translate('+(gmPyramidPlotWidth+80)+','+(50)+')')
...
{
    gPyramidPlotWidth = store.gPyramidPlotWidth;
    gPyramidPlotHeight = store.gPyramidPlotHeight;
    startYear = 2000
    store.currentYear = 2000
    endYear = 2050
    gSliderWidth = gPyramidPlotWidth/4;
    gSliderHeight = (gPyramidPlotHeight ) 
    gSlider = gPyramidPlot.append('g')
                    .attr('transform','translate('+gSliderWidth+','+gSliderHeight+')')
    slider = d3
        .sliderHorizontal()
        .min(startYear)
        .max(endYear)
                            ...
                            success: function(result)
                            {
                                ...
                                bars.append("rect")
                                    ...
                                    .on("mouseover",function(d){
                                        ...
                                        mWidth = d3.select("#m"+d.age).attr('width')
                                        fWidth = d3.select("#f"+d.age).attr('width')
                                        gLessGreater
...
/*function drawPyramidChart(countryName)
{
    store.countryName = countryName
    firstCallMale = store.firstCallMale
    firstCallFemale = store.firstCallFemale
